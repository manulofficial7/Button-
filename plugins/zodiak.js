...
const handler = (m, { usedPrefix, command, text }) => {
    if (!text) throw `contoh:\n${usedPrefix + command} වසරේ මාසයේ දිනය\n\n${usedPrefix + command} 2002 02 25`;
    ...
    const teks = `
උපත : ${birth.join('-')}
එළඹෙන උපන් දිනය : ${birthday.join('-')}
වත්මන් වයස : ${cekusia}
උපන්දිනය එනවා : ${cekusia + 1}
රාශි චක්‍රය : ${zodiac}
දැන් : ${WaktuSekarangReplit}
${monthsUntilNextBirthday} චන්ද්‍රයා ${daysUntilNextBirthday % 30} අද ඔබේ උපන් දිනයයි 🎂
${hoursUntilNextBirthday} තදබදය ${minutesUntilNextBirthday} මිනිත්තු ${secondsUntilNextBirthday} උපන්දිනයට තත්පර 🎉
`.trim();
    m.reply(teks);
};
