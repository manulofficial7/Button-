...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (!text) throw 'ආදාන විමසුම / Pinterest සබැඳිය'
    let res = await pinterest(text)
    // if (!res) throw res
    let mime = await lookup(res)
    let link = await shortUrl(res)
    text.match(URL_REGEX) ?
    await conn.sendMessage(m.chat, { [mime.split('/')[0]] : { url: res }, caption: `බාගත කිරීම සාර්ථකයි: ${await shortUrl(res)}` }, { quoted: m }) :
...
async function pinterest(query) {
    if (query.match(URL_REGEX)) {
        ...
        if (!data.length) throw `විමසුම "${query}" හමු වුණේ නැහැ :/`
